...
func (typeDescriptor *TypeDescriptor) parseDescByName() error {
	...
	err = typeDescriptor.unpack(util.Dm_build_922(buf))
	if err != nil {
		return err
	}
	return nil
...
func pack(typeDesc *TypeDescriptor, msg *util.Dm_build_912) error {
	switch typeDesc.column.colType {
	case ARRAY, SARRAY:
	...
	msg.Dm_build_1023(typeDesc.column.colType)

	msg.Dm_build_1023(typeDesc.column.prec)

	msg.Dm_build_1023(typeDesc.column.scale)
	return nil
}
...
func getPackArraySize(arrDesc *TypeDescriptor) (int, error) {
	...
	ret := util.Dm_build_586.Dm_build_793(name, serverEncoding)
	l += len(ret)
...
func packArray(arrDesc *TypeDescriptor, msg *util.Dm_build_912) error {

	msg.Dm_build_1023(arrDesc.column.colType)

	msg.Dm_build_1070(arrDesc.m_sqlName.m_name, arrDesc.getServerEncoding())

	msg.Dm_build_1023(int32(arrDesc.m_objId))

	msg.Dm_build_1023(int32(arrDesc.m_objVersion))

	msg.Dm_build_1023(int32(arrDesc.m_length))

	return pack(arrDesc.m_arrObj, msg)
}

func packRecord(strctDesc *TypeDescriptor, msg *util.Dm_build_912) error {

	msg.Dm_build_1023(strctDesc.column.colType)

	msg.Dm_build_1070(strctDesc.m_sqlName.m_name, strctDesc.getServerEncoding())

	msg.Dm_build_1023(int32(strctDesc.m_objId))

	msg.Dm_build_1023(int32(strctDesc.m_objVersion))

	msg.Dm_build_1020(int16(strctDesc.m_size))
...
func packClass(strctDesc *TypeDescriptor, msg *util.Dm_build_912) error {
	...
	if strctDesc.m_objId == 4 {
		...
		msg.Dm_build_1023(int32(strctDesc.m_outerId))

		msg.Dm_build_1023(int32(strctDesc.m_outerVer))

		msg.Dm_build_1023(int32(strctDesc.m_subId))
...
func (typeDescriptor *TypeDescriptor) unpack(buffer *util.Dm_build_912) error {

	typeDescriptor.column.colType = buffer.Dm_build_1084()
	...
	typeDescriptor.column.prec = buffer.Dm_build_1084()

	typeDescriptor.column.scale = buffer.Dm_build_1084()
	return nil
}

func (typeDescriptor *TypeDescriptor) unpackArray(buffer *util.Dm_build_912) error {

	typeDescriptor.m_sqlName.m_name = buffer.Dm_build_1126(typeDescriptor.getServerEncoding())

	typeDescriptor.m_sqlName.m_schId = int(buffer.Dm_build_1084())

	typeDescriptor.m_sqlName.m_packId = int(buffer.Dm_build_1084())

	typeDescriptor.m_objId = int(buffer.Dm_build_1084())

	typeDescriptor.m_objVersion = int(buffer.Dm_build_1084())

	typeDescriptor.m_length = int(buffer.Dm_build_1084())
...
func (typeDescriptor *TypeDescriptor) unpackRecord(buffer *util.Dm_build_912) error {
	...
	typeDescriptor.m_size = int(buffer.Dm_build_1094())
...
func (typeDescriptor *TypeDescriptor) unpackClnt_nestTab(buffer *util.Dm_build_912) error {

	typeDescriptor.m_maxCnt = int(buffer.Dm_build_1084())
...
func (typeDescriptor *TypeDescriptor) unpackClnt(buffer *util.Dm_build_912) error {

	typeDescriptor.m_outerId = int(buffer.Dm_build_1084())

	typeDescriptor.m_outerVer = int(buffer.Dm_build_1084())

	typeDescriptor.m_subId = int(buffer.Dm_build_1094())

	typeDescriptor.m_cltnType = int(buffer.Dm_build_1094())
...
func (typeDescriptor *TypeDescriptor) unpackClass(buffer *util.Dm_build_912) error {
...
func (typeDescriptor *TypeDescriptor) makeChkDesc_array(offset int, desc *TypeDescriptor) (int, error) {
	...
	util.Dm_build_586.Dm_build_597(typeDescriptor.m_descBuf, offset, ARRAY)
	offset += USINT_SIZE

	util.Dm_build_586.Dm_build_602(typeDescriptor.m_descBuf, offset, int32(desc.m_length))
	offset += ULINT_SIZE
...
func (typeDescriptor *TypeDescriptor) makeChkDesc_record(offset int, desc *TypeDescriptor) (int, error) {
	...
	util.Dm_build_586.Dm_build_597(typeDescriptor.m_descBuf, offset, PLTYPE_RECORD)
	offset += USINT_SIZE

	util.Dm_build_586.Dm_build_597(typeDescriptor.m_descBuf, offset, int16(desc.m_size))
	offset += USINT_SIZE
	var err error
	for i := 0; i < desc.m_size; i++ {
...
func (typeDescriptor *TypeDescriptor) makeChkDesc_buildin(offset int, desc *TypeDescriptor) int {
	...
	util.Dm_build_586.Dm_build_597(typeDescriptor.m_descBuf, offset, dtype)
	offset += USINT_SIZE

	util.Dm_build_586.Dm_build_597(typeDescriptor.m_descBuf, offset, int16(prec))
	offset += USINT_SIZE

	util.Dm_build_586.Dm_build_597(typeDescriptor.m_descBuf, offset, int16(scale))
	offset += USINT_SIZE
...
func (typeDescriptor *TypeDescriptor) makeChkDesc_class_clnt(offset int, desc *TypeDescriptor) (int, error) {
	...
	util.Dm_build_586.Dm_build_597(typeDescriptor.m_descBuf, offset, int16(desc.m_cltnType))
	offset += USINT_SIZE

	util.Dm_build_586.Dm_build_602(typeDescriptor.m_descBuf, offset, int32(desc.getMaxCnt()))
	offset += ULINT_SIZE
...
func (typeDescriptor *TypeDescriptor) makeChkDesc_class(offset int, desc *TypeDescriptor) (int, error) {
	...
	util.Dm_build_586.Dm_build_597(typeDescriptor.m_descBuf, offset, CLASS)
	offset += USINT_SIZE
	...
	if isClnt {
		util.Dm_build_586.Dm_build_587(typeDescriptor.m_descBuf, offset, byte(1))
	} else {
		util.Dm_build_586.Dm_build_587(typeDescriptor.m_descBuf, offset, byte(0))
	}
