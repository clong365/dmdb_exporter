...
const (
	...
	ZZ_BYTES_BUFFERSIZE = ZZ_BUFFERSIZE * 4
	/** lexical states */
	YYINITIAL = 0
	xc        = 2
	xq        = 4
...
func (lexer *Lexer) init() {
	...
	lexer.zzBytesBuffer = make([]byte, ZZ_BYTES_BUFFERSIZE)
	lexer.zzAtBOL = true
}

func (lexer *Lexer) Reset(in io.Reader) *Lexer {
	lexer.zzLexicalState = YYINITIAL
	lexer.zzAtBOL = true
	lexer.zzReader = in
	lexer.zzState = 0
 ...
 */
func NewLexer(in io.Reader, debug bool) *Lexer {
	l := new(Lexer)
	l.init()
	l.debugFlag = debug
	l.zzReader = in
...
func (lexer *Lexer) zzRefill() (bool, error) {
	...
	numRead, err := lexer.zzReader.Read(lexer.zzBytesBuffer)
	...
	j := lexer.zzEndRead
	for i < numRead {
		r, w := utf8.DecodeRune(lexer.zzBytesBuffer[i:])
		if r == utf8.RuneError {
			panic("DecodeRune error!")
		}
		lexer.zzBuffer[j] = r
		j++
		i += w
...
func (lexer *Lexer) yyclose() error {
	...
	if lexer.zzReader != nil {
		if c, ok := lexer.zzReader.(io.ReadCloser); ok {
			return c.Close()
		}
	}
	return nil
 ...
 */
func (lexer *Lexer) yyreset(reader io.ReadCloser) {
